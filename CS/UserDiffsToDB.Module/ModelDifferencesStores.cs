        ...
        protected virtual ModelDiffsBase GetModelDiffs(IObjectSpace objectSpace) {
            ...
            if (modelDiffs == null) {
                ...
                FileModelStore fileModelStore =
                    new FileModelStore(PathHelper.GetApplicationFolder(), AppDiffDefaultName);
                CopyAspectToDB(modelDiffs, fileModelStore, "");
                foreach (string aspectName in fileModelStore.GetAspects()) 
                    CopyAspectToDB(modelDiffs, fileModelStore, aspectName);    
                modelDiffs.Save();
            }
            return modelDiffs;
        }
        private void CopyAspectToDB(ModelDiffs modelDiffs, FileModelStore fileModelStore, string aspectName) {
            ModelAspect aspect = new ModelAspect(modelDiffs.Session);
            aspect.Aspect = aspectName;
            aspect.XmlData = File.ReadAllText(PathHelper.GetApplicationFolder() + 
