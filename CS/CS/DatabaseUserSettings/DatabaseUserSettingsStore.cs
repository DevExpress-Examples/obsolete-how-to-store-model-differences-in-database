    ...
    public class DatabaseUserSettingsStore : ModelDifferenceStore {
        ...
        public static IDatabaseUserSettings GetUserSettings(IObjectSpace objectSpace, Type userSettingsType, string userId) {
            ...
            if (string.IsNullOrEmpty(userId))
                ...
                ITypeInfo userTypeInfo = objectSpace.TypesInfo.FindTypeInfo(SecuritySystem.UserType);
                object user = objectSpace.FindObject(SecuritySystem.UserType, new OperandProperty(userTypeInfo.KeyMember.Name) == new OperandValue(userId));
                if (user != null)
                    if (Convert.ToString(userTypeInfo.FindMember(DatabaseUserSettingsModule.UserNameProperty).GetValue(user)) == DatabaseUserSettingsModule.ConfiguratorUserName)
                        userSettings = GetConfiguratorUserSettings(objectSpace, userSettingsType);
                    else
            ...
            if (userSettings == null) {
                ...
                foreach (string aspectName in aspects)
                    try {
                        ...
                        throw new UserFriendlyException(Localization.CannotLoadUserSettingsFromFile + Localization.UserSettingsFailureSuggestion);
                    }
            }
            return userSettings;
        }
        public override void Load(ModelApplicationBase model) {
            try {
                using (IObjectSpace objectSpace = Application.CreateObjectSpace()) {
                    ...
                    IDatabaseUserSettings userSettings = GetUserSettings(objectSpace, DatabaseUserSettingsModule.UserSettingsType, Convert.ToString(SecuritySystem.CurrentUserId));
                ...
                if (!(e is InvalidOperationException)) {
                    Tracing.Tracer.LogError(e);
                    throw new UserFriendlyException(Localization.CannotLoadUserSettingsFromTheDatabase + Localization.UserSettingsFailureSuggestion);
                }
            }
        }
        public override void SaveDifference(ModelApplicationBase model) {
            try {
                ...
                if (!(e is InvalidOperationException)) {
                    Tracing.Tracer.LogError(e);
                    throw new UserFriendlyException(Localization.CannotSaveUserSettingsToTheDatabase + Localization.UserSettingsFailureSuggestion);
                }
            }
        }
