    ...
    Public Class DatabaseModelStore
        ...
        Protected Overridable Function GetModelDiffs(ByVal objectSpace As IObjectSpace) As ModelDiffsBase
            ...
            If modelDiffs Is Nothing Then
                ...
                Dim fileModelStore As New FileModelStore(PathHelper.GetApplicationFolder(), AppDiffDefaultName)
                CopyAspectToDB(modelDiffs, fileModelStore, "")
                For Each aspectName As String In fileModelStore.GetAspects()
                    CopyAspectToDB(modelDiffs, fileModelStore, aspectName)
                Next aspectName
                modelDiffs.Save()
            End If
            Return modelDiffs
        ...
        Private Sub CopyAspectToDB(ByVal modelDiffs As ModelDiffsBase, ByVal fileModelStore As FileModelStore, ByVal aspectName As String)
            Dim aspect As New ModelAspect(modelDiffs.Session)
            aspect.Aspect = aspectName
            aspect.XmlData = File.ReadAllText(PathHelper.GetApplicationFolder() + _
                fileModelStore.GetFileNameForAspect(aspectName))
            modelDiffs.Aspects.Add(aspect)
        End Sub
